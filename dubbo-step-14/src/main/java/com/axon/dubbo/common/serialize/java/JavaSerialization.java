...
public class JavaSerialization implements Serialization {
    ...
    public byte[] serialize(Object obj) throws IOException {
        if (obj == null) throw new IllegalArgumentException("序列化对象不能为 null");
    ...
    public <T> T deserialize(byte[] bytes, Class<T> clazz) throws IOException {
        if (bytes == null || bytes.length == 0) throw new IllegalArgumentException("字节数组不能为空");
        ByteArrayInputStream bais = new ByteArrayInputStream(bytes);
        try (ObjectInputStream ois = new ObjectInputStream(bais)) {
            Object obj = ois.readObject();
            if (!clazz.isInstance(obj)) throw new IOException("类型不匹配");
            return (T) obj;
        } catch (ClassNotFoundException e) { throw new IOException("找不到类定义", e); }
    }
}
